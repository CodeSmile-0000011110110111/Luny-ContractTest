...
namespace Luny.ContractTest.LunyScriptTestsShouldNotBeInHere
{
	public abstract class LunyScriptWhenSelfContractTests : ContractTestBase
	{
		...
		public void SelfDisabled_DoesNotRunUpdates()
		{
			...
			Assert.That(gVars[nameof(LunyScript.LunyScript.When.Self.Steps)] == false);
			Assert.That(gVars[nameof(LunyScript.LunyScript.When.Self.Updates)] == false);
			Assert.That(gVars[nameof(LunyScript.LunyScript.When.Self.LateUpdates)] == false);
		}
		...
		public void SelfEnabled_RunsUpdates()
		{
			...
			Assert.That(gVars[nameof(LunyScript.LunyScript.When.Self.Steps)] == true);
			Assert.That(gVars[nameof(LunyScript.LunyScript.When.Self.Updates)] == true);
			Assert.That(gVars[nameof(LunyScript.LunyScript.When.Self.LateUpdates)] == true);
		}
	}
